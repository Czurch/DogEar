...
const nodesFromDocument = (document, xpathselector) =>
  xpath.select(xpathselector, document); // select specified key from doc
const mapProperties = (paths, document) =>
  Object.keys(paths).reduce(
    (acc, key) => ({ ...acc, [key]: nodesFromDocument(document, paths[key]) }),
    {}
  ); //for each key in xpaths, obtain the node from the document
...
mongoose.connect(
  "mongodb+srv://ferrel4:b0yBagelSr@cluster0.decaw.mongodb.net/dogear-users?retryWrites=true&w=majority"
);
...
app.delete("/api/bookmarks", async (req, res) => {
  ...
  try{
    ...
    if(user)
    {
      const updatedList = user.bookmarks.filter(bkmk => bkmk._id != bookmark._id);
      user.bookmarks = updatedList;
      user.save();
      res.json({status: "ok", bookmarks: user.bookmarks})
    }
  } catch (err) {
    return res.json({status: "error", error: "invalid token"});
  }
